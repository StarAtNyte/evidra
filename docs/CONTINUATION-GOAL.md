...
- Routine restart failures are now visible without a live daemon: expired runner leases make control-plane health blocked and create a critical `routine-stale` attention item, while completed-but-failed routines remain warning-level attention until inspected.
