...
- Agent lane wall-clock budgets now emit a durable `agent.lane.budget_exhausted` event at the crossing boundary, and operator attention marks the still-running lane as critical/blocked until its next safe-boundary stop or recovery.
