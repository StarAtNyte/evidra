...
- remote approval workflow: bearer-authenticated `GET /approvals` exposes the redacted operator inbox, while `POST /approvals/queue-task/:id/approve|reject` resolves queue gates without bypassing checkout validation; repeated decisions are safe no-ops;
