...
Benchmark runs can also enforce retention with `benchmark run --retention previous-report.json`: the same task-balanced, paired evidence gate checks that a harness change did not forget previously solved tasks. This protects against the harness-evolution failure mode described by [EVOHARNESSBENCH](https://arxiv.org/abs/2609.04280), where expanding capabilities can degrade prior competence.
...
Prediction artifacts can now be turned into targeted research evidence with `evidra evidence analyze predictions.json` or `/evidence analyze predictions.json` in the TUI. The typed `prediction.analyze` research tool reports classification confusion, regression residuals, metadata-defined worst slices, and binary calibration gaps from JSON/JSONL artifacts, so the director can form hypotheses about concrete failure slices rather than optimizing only an aggregate metric.
