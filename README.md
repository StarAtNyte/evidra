...
- durable routines: recurring research or challenge campaigns persist their goal, route, interval, per-run budget, explicit `--catch-up coalesce|replay` policy, optional `--max-runs` circuit breaker, last result, run history, and runner lease; replay retains at most eight missed event wakeups while coalesce collapses bursts, and the native routine daemon polls and executes due work without duplicate controllers;
