...
- remote operator notes: the bearer-authenticated `POST /tasks/note` control-plane endpoint provides the same handoff-note operation to dashboards, CI, and remote controllers without requiring a worker lease or worker credential; bounded `idempotencyKey` values make network retries safe;
