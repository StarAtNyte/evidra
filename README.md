...
- safe external wake-ups: `evidra event emit external.<source>.<event>` and `/event emit` accept bounded, redacted integration payloads, hash-chain them as non-evidence signals, and wake matching routines from CI, webhooks, schedulers, or competition monitors;
