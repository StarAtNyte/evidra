...
- a deterministic orchestration benchmark (`evidra benchmark orchestration`) covering duplicate lane prevention, lease ownership, queue ownership, dependency ordering, starvation prevention, deadline expiry, cancellation races, stale recovery, per-lane budget accounting, completion-proof enforcement, delegated-child completion gates, approval gates, queue-wide and per-task pause governance, priority and label control, and live budget stops;
