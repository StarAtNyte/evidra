...
- operator-set task budgets: `evidra queue budget <id> <tokens|0>` revises ceilings on queued or failed work without changing a live claim;
...
- a deterministic orchestration benchmark (`evidra benchmark orchestration`) covering duplicate lane prevention, lease ownership, queue ownership, dependency ordering, starvation prevention, deadline expiry, cancellation races, stale recovery, per-lane budget accounting, and completion-proof enforcement;
- optional queue completion contracts: declare `requiredPayloadKeys`, `requiredEvidenceRefs`, and `requiredActivityKinds` in a task payload so workers must produce verifiable proof before a task can become completed; rejected completions remain auditable and recoverable;
