...
- namespaced project plugins: `.evidra/plugins/<plugin-id>/plugin.json` contributes typed tools under `external.<plugin-id>.*`; plugins start quarantined until an operator enables them, then inherit the same role grants, manifest-drift detection, health circuit breaker, output quarantine, and audit trail as built-in adapters;
