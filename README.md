...
- **Schedule recovery visibility.** Expired routine leases and failed recurring runs appear in the same operator attention and health projection as failed tasks and stale agents, even when the routine daemon is offline.
- **Remote routine recovery.** Authenticated operators can recover one expired routine lease with `POST /routines/:id/recover`; worker credentials are rejected and non-stale retries return a conflict instead of stealing a live run.
- **Recurring-failure circuit breaker.** Failed routine runs use bounded exponential backoff so provider outages do not create retry storms; three consecutive failures pause the schedule, preserve the failure streak, and require an explicit operator resume instead of silently spending an unbounded campaign budget.
...
- Replay overflow is also an attention signal: dropped routine wakeups degrade control-plane health and appear with a direct catch-up policy remediation instead of remaining a passive counter.
...
- **Routine-level remote inspection.** `GET /routines/:id` exposes schedule state, bounded run history, associated routine-linked queue work, progress, and readiness so recurring research can be supervised and recovered remotely.
- Routine lineage survives `research --resume`: a routine-launched campaign retains its routine identity and attaches later cycle tickets to the same routine after a restart.
...
- **Fail-closed worker polling.** A worker exits its drain pass when all remaining tickets are blocked by approval, capabilities, assignment, dependencies, or deadlines; it does not spin or burn provider/CPU budget waiting for work another worker must unlock.
...
- durable queue approval gates and discussion: enqueue work with `requiresApproval`, release it with `evidra queue approve <id>` (or `/queue approve`), and attach bounded operator/worker comments with `evidra queue comment <id> "..."` (or `/queue comment`); rejected/pending tasks remain visible but cannot be claimed until explicitly approved;
- task work products: claimed workers can publish hashed, relative-path products through `context.publishWorkProduct(...)` or `/tasks/work-product`; products survive resume, appear in task detail/dashboard projections, and remain reviewable without treating an attached file as automatic proof;
...
- durable routines: recurring research or challenge campaigns persist their goal, route, interval, per-run budget, explicit `--catch-up coalesce|replay` policy, optional `--max-runs` circuit breaker, last result, run history, and runner lease; replay retains at most eight missed event wakeups, durably counts and audits overflow rather than hiding it, while coalesce collapses bursts, and the native routine daemon polls and executes due work without duplicate controllers;
...
- capacity-aware external workers: heartbeats may advertise a bounded concurrent-task capacity; remote checkout enforces that limit atomically, rejects claim-side escalation, persists the declaration across ordinary heartbeats, reports active/available slots, and leaves legacy workers unlimited when they do not declare one;
- resumable remote task context: successful `/tasks/claim` responses include bounded lineage, checkpoint metadata, attempt number, and recent redacted activity, so a reclaimed worker can continue durable work without replaying the entire event log;
...
- operator task conversation: `evidra queue note <task-id> "message"` and `/queue note <task-id> message` add an audited handoff note without changing task evidence, completion proof, or ownership;
- remote operator notes: the bearer-authenticated `POST /tasks/note` control-plane endpoint provides the same handoff-note operation to dashboards, CI, and remote controllers without requiring a worker lease or worker credential; bounded `idempotencyKey` values make network retries safe and conflicting reuse fails closed;
- remote operator cancellation: bearer-authenticated `POST /tasks/cancel` can stop queued or running work (including unfinished descendants) without impersonating a worker; the existing worker heartbeat returns the cooperative cancellation signal;
- remote queue governance: bearer-authenticated `POST /queue/pause` and `POST /queue/resume` control new dispatch during maintenance or incidents while preserving live-task state; worker credentials cannot change global queue governance;
- retry-safe queue governance: repeated pause/resume requests with the same state and reason are no-ops, preserving a truthful lifecycle audit under controller retries;
- remote queue read model: bearer-authenticated `GET /queue/status` returns redacted tasks, progress, readiness, usage, recovery alerts, pause state, and event-chain health for dashboards and controllers;
- remote projection safety: complete queue task projections—including payloads, progress messages, readiness, and usage metadata—and approval items pass through structured secret redaction before leaving the control plane; removing a claim token alone is not treated as sufficient isolation;
- remote activity feed: bearer-authenticated cursor-based `GET /activity?after=<event-id>&limit=<n>` streams bounded redacted activity pages with tamper-chain health, so dashboards can reconcile incrementally without reopening SQLite or polling an ever-growing snapshot;
- remote organization projection: bearer-authenticated `GET /organization` exposes the redacted campaign, agent reporting lines, role controls, external workers, recurring routines, queue control, project identity, and integrity state for remote supervision;
- remote directive visibility: the organization projection includes bounded pending specialist directive summaries, preserving source actor and scope so remote operators can verify steering before delivery;
- remote routine governance: bearer-authenticated `POST /routines/:id/pause` and `/routines/:id/resume` let an operator stop or restart recurring research safely; running routines refuse state mutation and unknown ids return clear errors;
- remote routine wake-up: bearer-authenticated `POST /routines/:id/trigger` requests an immediate autonomous cycle, coalescing repeated triggers while a run is active instead of launching concurrent campaigns;
- remote routine policy control: bearer-authenticated `POST /routines/:id/catch-up?policy=coalesce|replay` changes a non-running routine between `coalesce` and bounded `replay`, normalizing pending work and recording the operator change in the audit chain;
- remote agent governance: bearer-authenticated `POST /agents/:role/pause`, `/resume`, `/terminate`, and `/revive` control known roles at safe boundaries, preserve role-control audit events, reject workers, and make retries no-ops;
- remote specialist steering: bearer-authenticated `POST /agents/:role/message` queues a bounded, optionally phase-scoped directive for delivery at the next safe role boundary; identical pending retries are deduplicated, and steering cannot expand role authority or bypass admission;
- remote directive cancellation: bearer-authenticated `POST /agents/:role/directives/:id/cancel` withdraws an unapplied directive with role ownership checks; repeated cancellation is a safe no-op and applied directives remain immutable;
- remote directive history: bearer-authenticated `GET /agents/:role/directives` returns bounded redacted pending/applied/cancelled steering history with event-chain health;
- remote approval workflow: bearer-authenticated `GET /approvals` exposes the redacted operator inbox, while `POST /approvals/queue-task/:id/approve|reject` and `/approvals/agent-role/:role/approve|reject` resolve queue and external-role gates without bypassing checkout or admission validation; repeated decisions are safe no-ops;
- remote actor attribution: governance requests may send bounded `X-Evidra-Actor`; the label is carried into durable approval, cancellation, handoff-note, queue, and role-control audit records, defaulting safely to `remote-operator`;
...
- governed project adapters: `.evidra/tools.json` extends the typed research registry with argv-only domain tools; explicit role grants, bounded execution, manifest fingerprints, automatic manifest-drift quarantine, output hashes, injection-signal quarantine, lifecycle controls, approval-inbox review, durable health probes with consecutive-failure quarantine, and portable redacted state keep extensions accountable without forking Evidra;
- namespaced project plugins: `.evidra/plugins/<plugin-id>/plugin.json` contributes typed tools under `external.<plugin-id>.*`; plugins start quarantined until an operator enables them, then inherit the same role grants, manifest-drift detection, health circuit breaker, output quarantine, and audit trail as built-in adapters; see [the plugin contract](docs/plugins.md);
...
- a deterministic agent-governance benchmark (`evidra benchmark governance`) covering role-contract completeness, explicit custom-role and plugin admission/rejection, tool authority boundaries, pause/resume controls, scoped handoff isolation, durable handoff auditability, and recovery approval boundaries;
