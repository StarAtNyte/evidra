...
- explicit agent evaluation: `evidra agents evaluate` or `/agents evaluate` scores observed role trajectories and persists bounded preserve/coach/observe interventions; add `--apply` or use `/agents evaluate apply` to turn coach findings into durable role directives delivered at the next safe boundary; add `--json` for integrations;
