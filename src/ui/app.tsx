...
export function App({ root }: { root: string }): React.JSX.Element {
  ...
  const runResearchCycle = async (objective: string, campaign?: ResearchCampaign, parentTaskId?: string): Promise<{ text: string; goalStatus: "active" | "blocked" | "met"; decision: "inspect" | "propose" | "run" | "replicate" | "stop" }> => {
    ...
    if (executionPlaybook) trajectoryStore.appendEvent("research.execution.playbook", executionPlaybook);
  ...
  const runAutonomousCycle = async (campaignOverride?: ResearchCampaign, autoContinue = false): Promise<void> => {
    ...
    try {
      ...
      let stopPolicyAction: "continue" | "pause" | "stop" = "continue";
      if (campaign) {
        ...
        const rewards = stopCheck.eventsByType("research.search.reward").flatMap((event) => {
          const payload = event.payload && typeof event.payload === "object" ? event.payload as { reward?: unknown; durationSeconds?: unknown; valid?: unknown } : {};
          const reward = Number(payload.reward);
          return Number.isFinite(reward) ? [{ reward, durationSeconds: Number(payload.durationSeconds), valid: payload.valid !== false }] : [];
        });
        const stopPolicy = assessStopPolicy({
          ...
          openFalsifications: openFalsificationCount,
        });
        stopPolicyAction = stopPolicy.action;
        stopCheck.appendEvent("research.stop_policy.assessed", { ...stopPolicy, source: "tui" });
        if (openFalsificationCount > 0 && (cycle.decision === "stop" || stopPolicy.action === "stop")) stopCheck.appendEvent("research.stop_policy.continued", { reason: "open falsification agenda", openFalsifications: openFalsificationCount, source: "tui" });
        stopCheck.close();
      }
      const stopRequested = cycle.decision === "stop" || stopPolicyAction === "stop";
      if (campaign && !approvalRequired && stopRequested && stopPolicyAction !== "pause" && openFalsificationCount === 0) {
        campaign.status = "completed";
        persistCampaign(campaign);
        setConfig((current) => ({ ...current, campaign: { ...campaign, status: "completed" } }));
        if (loopTimer.current) { clearInterval(loopTimer.current); loopTimer.current = null; }
        ...
        if (loopTimer.current) { clearInterval(loopTimer.current); loopTimer.current = null; }
        append("assistant", "Autonomous research paused by the stop policy. Inspect the failure/leakage evidence, then use /research resume.");
      } else if (campaign && !approvalRequired && stopRequested && openFalsificationCount > 0) {
  ...
  const submit = async (value: string, fromQueue = false): Promise<void> => {
    ...
    if (routineSetupStep) {
      ...
      if (routineSetupStep === "trigger") {
        ...
        setRoutineSetupDraft((current) => ({ ...current, triggerEvent })); setRoutineSetupStep("catchUp"); append("assistant", routineSetupPrompt("catchUp")); return;
      }
      const catchUpPolicy = request.trim().toLowerCase();
      if (catchUpPolicy !== "coalesce" && catchUpPolicy !== "replay") {
        append("assistant", "Use coalesce or replay."); return;
      }
      ...
      const entry = store.createRoutine({
        ...
        budgetMinutes: draft.budgetMinutes ?? 60, intervalSeconds: draft.intervalSeconds ?? 86_400, triggerEvent: draft.triggerEvent, stopCondition: "stop when the stated goal has sufficient reproducible evidence",
        provider: configRef.current.provider, model: configRef.current.model, thinking: configRef.current.reasoningEffort,
        autonomy: configRef.current.autonomy, limitPolicy: configRef.current.limitPolicy, executor: configRef.current.experimentExecutor, lanes: 3, maxRuns: null, catchUpPolicy,
      });
      store.close();
    ...
    const contractAgentMatch = request.match(/^\/agents\s+contract\s+(.+?)\s+--responsibility\s+(.+?)\s+--authority\s+(coordinate|investigate|validate|execute|repair)\s+--playbook\s+(.+?)(?:\s+--tools\s+(.+?))?(?:\s+--skills\s+(.+?))?(?:\s+--parent\s+(.+?))?(\s+--trusted)?$/i);
    if (contractAgentMatch) {
      ...
      try {
        const contract = store.setAgentRoleContract({
          role,
          parentRole: contractAgentMatch[7]?.trim() || "research director",
          ...
          reviewRequired: !Boolean(contractAgentMatch[8]),
          ...
          ...(contractAgentMatch[6] ? { skillAllowlist: contractAgentMatch[6].split(",").map((skill) => skill.trim()).filter(Boolean) } : {}),
        });
    ...
    if (request === "/agents activity") {
      const store = new ResearchStore(join(root, ".sota", "database.sqlite"));
      const activity = store.agentActivities({ limit: 32 }).slice().reverse();
      store.close();
    ...
    if (request === "/agents dispatch") {
      const store = new ResearchStore(join(root, ".sota", "database.sqlite"));
      const event = store.eventsByType("research.lane.dispatch_planned", 1).at(-1);
      store.close();
      if (!event) { append("assistant", "No research lane dispatch plan recorded."); return; }
    ...
    if (request === "/agents evaluate" || request === "/agents evaluate apply") {
      const store = new ResearchStore(join(root, ".sota", "database.sqlite"));
    ...
    if (request === "/agents" || request === "/agents status" || request === "/agents limits") {
      ...
      store.close();
      append("assistant", `Research agents\n  codex: ${codex || "not authenticated"}\n  local: ${local}\n  concurrency: 1 active director lane\n\n${lanes.length ? lanes.map((lane) => { const pause = pauses.get(lane.role); const directives = pendingDirectives.get(lane.role) ?? 0; const heartbeat = lane.heartbeatAt ? Date.parse(lane.heartbeatAt) : Number.NaN; const health = lane.status === "running" ? (Number.isFinite(heartbeat) && Date.now() - heartbeat <= 120_000 ? "healthy" : "stale") : lane.status === "idle" ? "idle" : "unstarted"; return `  ${pause?.terminated ? "×" : pause?.paused ? "Ⅱ" : lane.status === "running" ? "●" : lane.status === "failed" ? "✗" : lane.status === "blocked" ? "!" : "○"} ${lane.role} · ${pause?.terminated ? "terminated" : pause?.paused ? "paused" : lane.status} · health ${health} · ${lane.provider}/${lane.model}${directives ? ` · ${directives} directive(s)` : ""}${pause?.reason ? ` · ${pause.reason}` : ""}${lane.task ? `\n    ${lane.task.slice(0, 120)}` : ""}`; }).join("\n") : "  No lanes initialized; start /research to initialize the project."}`);
    ...
    if (request === "/queue" || request === "/queue status" || request.startsWith("/queue status ") || request === "/queue pause" || request.startsWith("/queue pause ") || request === "/queue resume" || request.startsWith("/queue priority ") || request.startsWith("/queue labels ") || request.startsWith("/queue pause-task ") || request.startsWith("/queue resume-task ") || request.startsWith("/queue approve ") || request.startsWith("/queue reject ") || request.startsWith("/queue history ") || request.startsWith("/queue checkpoint ") || request === "/queue usage" || request.startsWith("/queue usage ") || request === "/queue recover" || request.startsWith("/queue recover ") || request.startsWith("/queue activity ") || request.startsWith("/queue products ") || request.startsWith("/queue verify-products ") || request.startsWith("/queue assign ") || request.startsWith("/queue budget ") || request.startsWith("/queue cost-budget ") || request.startsWith("/queue deadline ") || request.startsWith("/queue contract ") || request.startsWith("/queue cancel ") || request.startsWith("/queue note ") || request.startsWith("/queue comment ")) {
      ...
      const productsMatch = request.match(/^\/queue products\s+(\S+)$/);
      const verifyProductsMatch = request.match(/^\/queue verify-products\s+(\S+)$/);
      const historyMatch = request.match(/^\/queue history\s+(\S+)$/);
      const checkpointMatch = request.match(/^\/queue checkpoint\s+(\S+)$/);
      ...
      const pauseTaskMatch = request.match(/^\/queue pause-task\s+(\S+)(?:\s+--reason\s+(.+))?$/);
      const resumeTaskMatch = request.match(/^\/queue resume-task\s+(\S+)$/);
      ...
      const commentMatch = request.match(/^\/queue comment\s+(\S+)\s+(.+)$/);
      ...
      if (historyMatch) {
        const history = store.queueHistory(historyMatch[1], 64);
        append("assistant", history.length ? history.map((entry) => `${entry.createdAt}  ${entry.type}`).join("\n") : `No lifecycle history recorded for ${historyMatch[1]}.`);
      } else if (checkpointMatch) {
        const checkpoint = store.queueCheckpoint(checkpointMatch[1]);
        if (!checkpoint) append("assistant", `Task ${checkpointMatch[1]} was not found.`);
        else append("assistant", `Task ${checkpointMatch[1]}\nCheckpoint ${checkpoint.present ? "available" : "none"}${checkpoint.present ? `\nStage      ${checkpoint.stage ?? "unknown"}\nBytes      ${checkpoint.bytes}\nHash       ${checkpoint.hash}\nKeys       ${checkpoint.keys.join(", ") || "none"}\nUpdated    ${checkpoint.updatedAt}` : ""}`);
      } else if (labelsMatch) {
        try {
          const labels = /^none$/i.test(labelsMatch[2].trim()) ? [] : labelsMatch[2].split(",").map((label) => label.trim()).filter(Boolean);
          if (!store.setTaskLabels(labelsMatch[1], labels)) throw new Error("task is missing, running, or terminal");
          append("assistant", labels.length ? `Set labels for ${labelsMatch[1]}: ${labels.join(", ")}.` : `Cleared labels for ${labelsMatch[1]}.`);
        } catch (error) { append("assistant", `Queue label update failed: ${error instanceof Error ? error.message : String(error)}`); }
        ...
        } catch (error) { append("assistant", `Queue task pause failed: ${error instanceof Error ? error.message : String(error)}`); }
      } else if (resumeTaskMatch) {
        try {
          if (!store.resumeTask(resumeTaskMatch[1])) throw new Error("task is missing or not paused");
          append("assistant", `Resumed queue task ${resumeTaskMatch[1]}.`);
        } catch (error) { append("assistant", `Queue task resume failed: ${error instanceof Error ? error.message : String(error)}`); }
      } else if (request === "/queue pause" || pauseMatch) {
        ...
        const products = store.queueWorkProducts(productsMatch[1], 32);
        append("assistant", products.length ? products.map((product) => `${product.createdAt}  ${product.name} · ${product.actorId} · ${product.verification}\n  ${product.path}\n  ${product.checksum}`).join("\n") : `No work products recorded for ${productsMatch[1]}.`);
      } else if (verifyProductsMatch) {
        const products = store.queueWorkProducts(verifyProductsMatch[1], 128).map((product) => store.verifyQueueWorkProduct(root, product.id)).filter((product): product is NonNullable<typeof product> => product !== undefined);
        append("assistant", products.length ? products.map((product) => `${product.verification === "verified" ? "OK" : "FAIL"} ${product.name} · ${product.verification}\n  ${product.path}\n  expected ${product.checksum}`).join("\n") : `No work products recorded for ${verifyProductsMatch[1]}.`);
      } else if (usageMatch) {
        ...
        try {
          ...
          append("assistant", assignMatch[2] ? `Assigned ${assignMatch[1]} to ${assignMatch[2]}.` : `Cleared assignment for ${assignMatch[1]}.`);
        } catch (error) { append("assistant", `Queue assignment failed: ${error instanceof Error ? error.message : String(error)}`); }
      } else if (budgetMatch) {
        const value = /^(?:0|unlimited)$/i.test(budgetMatch[2]) ? null : Number(budgetMatch[2]);
        try {
          if (value !== null && (!Number.isFinite(value) || !Number.isInteger(value) || value <= 0)) throw new Error("budget must be a positive integer, 0, or unlimited");
        ...
        let contract = null;
        if (!/^none$/i.test(contractMatch[2])) {
          try {
            const parsed = JSON.parse(contractMatch[2]);
            if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("contract must be a JSON object");
            contract = parsed;
          } catch (error) { append("assistant", `Queue contract update failed: ${error instanceof Error ? error.message : String(error)}`); contract = undefined; }
        }
        if (contract !== undefined) {
          try {
            if (!store.setTaskCompletionContract(contractMatch[1], contract)) throw new Error("task is missing or not queued/failed");
            append("assistant", contract === null ? `Cleared completion contract for ${contractMatch[1]}.` : `Updated completion contract for ${contractMatch[1]}.`);
          } catch (error) { append("assistant", `Queue contract update failed: ${error instanceof Error ? error.message : String(error)}`); }
        }
      } else if (noteMatch) {
        ...
        const recorded = store.recordQueueActivity({ taskId: commentMatch[1], actorId: "operator", kind: "comment", message: commentMatch[2] });
        append("assistant", recorded ? `Added a discussion comment to ${commentMatch[1]}.` : `Unable to add a comment to ${commentMatch[1]}.`);
      } else if (cancelMatch) {
        try {
          if (!store.cancelTask(cancelMatch[1], cancelMatch[2] ?? "operator cancelled task")) throw new Error("task is missing or already terminal");
          append("assistant", `Cancelled ${cancelMatch[1]}.`);
        } catch (error) { append("assistant", `Queue cancellation failed: ${error instanceof Error ? error.message : String(error)}`); }
    ...
    if (request === "/research" || request.startsWith("/research ")) {
      ...
      if (objective === "examples") {
        append("assistant", `Research starter briefs\n\n${researchStarterText()}\n\nAdapt any brief into /research start. Evidra will define the internal phase goals, validation contract, and stopping condition for the final objective.`);
        return;
      }
