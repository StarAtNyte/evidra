...
function render(d){
 ...
 const map=d.organizationMap||{}; const phases=(map.phases||[]).map(p=>row(esc(p.phase)+' · '+esc(p.id),status(p.status)+' · '+esc(p.queue.active)+' active / '+esc(p.queue.total)+' queued'+(p.queue.blocked?' · '+esc(p.queue.blocked)+' blocked':'')+(p.objective?' · '+esc(p.objective):''))).join(''); const roles=(map.roles||[]).filter(r=>r.parentRole===null||r.status!=='unstarted'||r.pendingDirectives||r.activeQueue).map(r=>row(esc(r.role)+' · '+esc(r.parentRole||'operator'),status(r.status)+' · '+esc(r.health)+(r.activeQueue?' · '+esc(r.activeQueue)+' active':'')+(r.pendingDirectives?' · '+esc(r.pendingDirectives)+' directives':'')+(r.task?' · '+esc(r.task):''))).join(''); if(organizationPanel) organizationPanel.innerHTML=row('Mission',esc(map.goal||'not initialized'))+row('Shape',esc(map.mode)+' · '+esc(map.totals?.roles||0)+' roles · '+esc(map.totals?.phases||0)+' phases')+row('Work',esc(map.totals?.activeQueue||0)+' active · '+esc(map.totals?.queue||0)+' aligned'+(map.totals?.blockedQueue?' · '+esc(map.totals.blockedQueue)+' blocked':''))+(phases?'<div class="sub">phase ownership</div>'+phases:'')+(roles?'<div class="sub">active reporting lines</div>'+roles:'');
