...
</style></head><body><main><header><div><h1><span>EVIDRA</span> / DASHBOARD</h1><div class="sub">Read-only local view of durable research state</div></div><div class="refresh" id="updated">Connecting…</div></header><section class="grid" id="stats"></section><div class="layout"><div><section class="panel"><h2>Campaign</h2><div id="campaign"></div></section><section class="panel"><h2>Research stages</h2><div id="stages"></div></section><section class="panel"><h2>Research phases</h2><div id="phases"></div></section><section class="panel"><h2>Experiments & runs</h2><div id="work"></div></section></div><div><section class="panel"><h2>Research agents</h2><div id="agents"></div></section><section class="panel"><h2>Inference cost by work item</h2><div id="agent-cost"></div></section><section class="panel"><h2>Recurring routines</h2><div id="routines"></div></section><section class="panel"><h2>Approval inbox</h2><div id="approvals"></div></section><section class="panel"><h2>Work queue</h2><div id="queue"></div></section><section class="panel"><h2>Recent events</h2><pre id="events"></pre></section></div></div></main>
<script>
...
function render(d){
 ...
 const reviews=new Map((d.agentReviews||[]).map(x=>[x.role,x])); document.getElementById('agents').innerHTML=(d.organization||d.agents||[]).map(a=>{const review=reviews.get(a.role);const checks=review?(' · checks '+(review.playbookPasses||0)+' pass / '+(review.playbookPartials||0)+' partial / '+(review.playbookBlocks||0)+' blocked'):'';const directives=a.pendingDirectives?' · directives '+a.pendingDirectives:'';const playbook=(a.playbook||[]).slice(0,2).map(esc).join(' · ');return row(esc(a.role)+' · '+esc(a.parentRole||'operator'),a.control?.terminated?status('terminated'):a.control?.paused?status('paused'):status(a.status)+(review?' · review '+esc(review.recommendation)+' '+Math.round((review.score||0)*100)+'%':'')+checks+directives+(a.task?' · '+esc(a.task):'')+(a.budgetSeconds!==null&&a.budgetSeconds!==undefined?' · '+Math.round(a.usedSeconds||0)+'/'+Math.round(a.budgetSeconds)+'s':'') )+(playbook?'<div class="sub">playbook: '+playbook+'</div>':'')}).join('')||empty;
 const workers=(d.externalWorkers||[]).map(w=>row(esc(w.role)+' · '+esc(w.workerId),status(w.status)+' · '+esc(w.provider)+'/'+esc(w.model)+(w.capabilities?.length?' · '+w.capabilities.map(esc).join(', '):'')+(w.task?' · '+esc(w.task):'')+' · '+age(w.lastHeartbeatAt))).join(''); if(workers) document.getElementById('agents').innerHTML+='<div class="sub">external workers</div>'+workers;
 ...
 const reviewHistory=(d.agentReviewHistory||[]).slice().reverse().slice(0,6).map(h=>'<div class="sub">review '+esc(h.createdAt)+' · '+esc(h.source||'controller')+(h.objective?' · '+esc(h.objective):'')+'<br>'+((h.interventions||[]).map(i=>esc((i.role||'role')+' → '+(i.action||'observe')+' · '+(i.priority||'normal'))).join(' · ')||'no interventions')+'</div>').join('')||empty; const reviewHistoryEl=document.getElementById('agent-review-history')||document.getElementById('agents'); if(reviewHistoryEl) reviewHistoryEl.innerHTML+=(reviewHistory?' <div class="sub">review history</div>'+reviewHistory:'');
