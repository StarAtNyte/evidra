...
</style></head><body><main><header><div><h1><span>EVIDRA</span> / DASHBOARD</h1><div class="sub">Read-only local view of durable research state</div></div><div class="refresh" id="updated">Connecting…</div></header><section class="grid" id="stats"></section><div class="layout"><div><section class="panel"><h2>Campaign</h2><div id="campaign"></div></section><section class="panel"><h2>Research stages</h2><div id="stages"></div></section><section class="panel"><h2>Research phases</h2><div id="phases"></div></section><section class="panel"><h2>Experiments & runs</h2><div id="work"></div></section></div><div><section class="panel"><h2>Research agents</h2><div id="agents"></div></section><section class="panel"><h2>Recurring routines</h2><div id="routines"></div></section><section class="panel"><h2>Work queue</h2><div id="queue"></div></section><section class="panel"><h2>Recent events</h2><pre id="events"></pre></section></div></div></main>
<script>
...
function render(d){
 ...
 document.getElementById('routines').innerHTML=(d.routines||[]).map(r=>row(esc(r.name)+' · '+esc(r.mode),status(r.status)+' · next '+esc(r.nextRunAt)+' · '+esc(r.runCount)+' run'+(r.runCount===1?'':'s')+(r.lastResult?' · last '+esc(r.lastResult):'')+(r.lastError?' · '+esc(r.lastError):''))).join('')||empty;
