...
function render(d){
 ...
 const map=d.organizationMap||{}; const progress=map.progress||{}; const phases=(map.phases||[]).map(p=>row(esc(p.phase)+' · '+esc(p.id),status(p.status)+' · '+esc(p.queue.active)+' active / '+esc(p.queue.queued)+' queued / '+esc(p.queue.completed)+' done'+(p.queue.blocked?' · '+esc(p.queue.blocked)+' blocked':'')+(p.queue.failed?' · '+esc(p.queue.failed)+' failed':'')+' · '+esc((p.usage?.inputTokens||0)+(p.usage?.outputTokens||0))+' tokens / $'+Number(p.usage?.costUsd||0).toFixed(4)+(p.budget?.tokenUtilization!==null&&p.budget?.tokenUtilization!==undefined?' · token '+Math.round(p.budget.tokenUtilization*100)+'%':'')+(p.budget?.costUtilization!==null&&p.budget?.costUtilization!==undefined?' · cost '+Math.round(p.budget.costUtilization*100)+'%':'')+(p.objective?' · '+esc(p.objective):''))).join(''); const roles=(map.roles||[]).filter(r=>r.parentRole===null||r.status!=='unstarted'||r.pendingDirectives||r.activeQueue).map(r=>row(esc(r.role)+' · '+esc(r.parentRole||'operator'),status(r.status)+' · '+esc(r.admission||'approved')+' · '+esc(r.authority||'unknown')+' · tools '+esc((r.toolAllowlist||[]).join(',')||'defaults')+' · '+esc(r.health)+(r.activeQueue?' · '+esc(r.activeQueue)+' active':'')+(r.pendingDirectives?' · '+esc(r.pendingDirectives)+' directives':'')+(r.task?' · '+esc(r.task):''))).join(''); const accountability=map.accountability||{}; if(organizationPanel) organizationPanel.innerHTML=row('Mission',esc(map.goal||'not initialized'))+row('Shape',esc(map.mode)+' · '+esc(map.totals?.roles||0)+' roles · '+esc(map.totals?.phases||0)+' phases')+row('Goal progress',esc(progress.completedPhases||0)+'/'+esc(progress.totalPhases||0)+' phases · '+esc(Math.round((progress.ratio||0)*100))+'% · '+esc(progress.status||'pending')+(progress.activePhase?' · active '+esc(progress.activePhase):''))+row('Work',esc(map.totals?.activeQueue||0)+' active · '+esc(map.totals?.queuedQueue||0)+' queued · '+esc(map.totals?.completedQueue||0)+' done · '+esc(map.totals?.queue||0)+' aligned'+(map.totals?.unscopedQueue?' · '+esc(map.totals.unscopedQueue)+' unscoped':'')+(map.totals?.blockedQueue?' · '+esc(map.totals.blockedQueue)+' blocked':'')+(map.totals?.failedQueue?' · '+esc(map.totals.failedQueue)+' failed':''))+row('Usage',esc((map.totals?.usage?.inputTokens||0)+(map.totals?.usage?.outputTokens||0))+' tokens · $'+Number(map.totals?.usage?.costUsd||0).toFixed(4)+(map.totals?.budget?.tokenUtilization!==null&&map.totals?.budget?.tokenUtilization!==undefined?' · token budget '+Math.round(map.totals.budget.tokenUtilization*100)+'%':'')+(map.totals?.budget?.costUtilization!==null&&map.totals?.budget?.costUtilization!==undefined?' · cost budget '+Math.round(map.totals.budget.costUtilization*100)+'%':''))+row('Accountability',esc((accountability.unassignedRunning||[]).length)+' ownerless · '+esc((accountability.unscopedLive||[]).length)+' unscoped · '+esc((accountability.misalignedLive||[]).length)+' mis-scoped · '+esc((accountability.unbudgetedLive||[]).length)+' unbudgeted'+(accountability.foreignCampaignLive?.length?' · '+esc(accountability.foreignCampaignLive.length)+' foreign campaign':'')+(accountability.legacyCampaignLive?.length?' · '+esc(accountability.legacyCampaignLive.length)+' legacy campaign':'') )+(phases?'<div class="sub">phase ownership</div>'+phases:'')+(roles?'<div class="sub">active reporting lines</div>'+roles:'');
