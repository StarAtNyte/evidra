...
export function dashboardSnapshot(store: ResearchStore): Record<string, unknown> {
  ...
  const snapshot = {
    ...
    organization: agentOrganization(store).map((agent) => ({ ...agent, control: store.agentPause(agent.role) ?? null })),
...
function render(d){
 ...
 const reviews=new Map((d.agentReviews||[]).map(x=>[x.role,x])); document.getElementById('agents').innerHTML=(d.organization||d.agents||[]).map(a=>{const review=reviews.get(a.role);const checks=review?(' · checks '+(review.playbookPasses||0)+' pass / '+(review.playbookPartials||0)+' partial / '+(review.playbookBlocks||0)+' blocked'):'';const playbook=(a.playbook||[]).slice(0,2).map(esc).join(' · ');return row(esc(a.role)+' · '+esc(a.parentRole||'operator'),a.control?.paused?status('paused'):status(a.status)+(review?' · review '+esc(review.recommendation)+' '+Math.round((review.score||0)*100)+'%':'')+checks+(a.task?' · '+esc(a.task):'')+(a.budgetSeconds!==null&&a.budgetSeconds!==undefined?' · '+Math.round(a.usedSeconds||0)+'/'+Math.round(a.budgetSeconds)+'s':'') )+(playbook?'<div class="sub">playbook: '+playbook+'</div>':'')}).join('')||empty;
