...
import { agentBudgetLedger, summarizeAgentUsageBy } from "./usage.js";
...
function render(d){
 ...
 document.getElementById('routines').innerHTML=(d.routines||[]).map(r=>row(esc(r.name)+' · '+esc(r.mode),status(r.status)+' · next '+esc(r.nextRunAt)+' · trigger '+esc(r.triggerEvent||'none')+(r.lastTriggerAt?' · triggered '+esc(r.lastTriggerAt):'')+' · '+esc(r.runCount)+' run'+(r.runCount===1?'':'s')+(r.lastResult?' · last '+esc(r.lastResult):'')+(r.lastError?' · '+esc(r.lastError):'')+(r.recentRuns?.length?' · history '+r.recentRuns.map(x=>esc(x.status)).join(' → '):''))).join('')||empty;
