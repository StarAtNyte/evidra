...
function render(d){
 ...
 const workers=(d.externalWorkers||[]).map(w=>row(esc(w.role)+' · '+esc(w.workerId),status(w.status)+' · '+esc(w.admission||'approved')+' · '+esc(w.provider)+'/'+esc(w.model)+(w.capabilities?.length?' · '+w.capabilities.map(esc).join(', '):'')+(w.task?' · '+esc(w.task):'')+' · '+age(w.lastHeartbeatAt))).join(''); if(workers) document.getElementById('agents').innerHTML+='<div class="sub">external workers</div>'+workers;
