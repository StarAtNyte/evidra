...
export function prepareSubmission(root: string, experimentId: string, manifest: ExperimentManifest, run: RunResult, competition: CompetitionConfig): { id: string; path: string } {
  ...
  if (!copied.length) throw new Error(`Run ${run.runId} has no submission or prediction artifact.`);
...
export function validateSubmissionBundle(path: string): SubmissionValidation {
  ...
  let provenance: { submissionId?: unknown; experimentId?: unknown } | undefined;
  ...
  checks.push({ name: "prediction-artifact", passed: predictionArtifact, detail: predictionArtifact ? "submission or prediction artifact is present" : "no submission or prediction artifact is present" });
