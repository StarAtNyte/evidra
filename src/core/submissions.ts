...
export function prepareSubmission(root: string, experimentId: string, manifest: ExperimentManifest, run: RunResult, competition: CompetitionConfig): { id: string; path: string } {
  ...
  const provenance = redactStructured({ schemaVersion: 1, submissionId: id, experimentId, competition: competition.id, submission, manifest, run: { runId: run.runId, status: run.status, metrics: run.metrics, metricsByFold: run.metricsByFold, artifacts: run.artifacts }, createdAt: new Date().toISOString() });
