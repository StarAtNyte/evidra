...
export function summarizeTimelineEvent(event: TimelineEvent): string {
  ...
  if (event.type === "ensemble.candidate.created" || event.type === "ensemble.candidate.recorded") return `ensemble · candidate ${text(payload.id, "created")}${payload.checksum ? ` · ${text(payload.checksum, "checksummed")}` : ""}`;
  if (event.type === "ensemble.candidate.status") return `ensemble · ${text(payload.id, "candidate")} · ${text(payload.status, "status")}`;
