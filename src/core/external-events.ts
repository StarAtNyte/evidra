...
export interface ExternalAgentHeartbeat {
  ...
  /** Maximum number of queue tasks this worker can own concurrently. */
...
export function parseExternalAgentHeartbeat(payload: Record<string, unknown>): ExternalAgentHeartbeat {
  ...
  if (payload.capacity !== undefined && payload.capacity !== null && (typeof payload.capacity !== "number" || !Number.isInteger(payload.capacity) || payload.capacity < 1 || payload.capacity > 64)) throw new Error("External agent heartbeat capacity must be an integer from 1 to 64.");
  ...
  const capacity = Object.prototype.hasOwnProperty.call(payload, "capacity") ? (typeof payload.capacity === "number" ? payload.capacity : null) : undefined;
  return { ...(workspaceId ? { workspaceId } : {}), role, leaseId, provider, model, status: status as ExternalAgentHeartbeat["status"], task: typeof payload.task === "string" ? payload.task : null, budgetSeconds: typeof payload.budgetSeconds === "number" ? payload.budgetSeconds : null, ...(capacity === undefined ? {} : { capacity }), capabilities };
}
