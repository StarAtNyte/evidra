...
agents.command("recover").description("Reclaim internal agent leases and specialist tickets whose heartbeats expired").action(() => {
  ...
  if (tickets.length || roles.length || directives.length) store.appendEvent("research.agent.recovery.completed", { tickets, roles, directives, source: "operator-cli" });
  store.close();
...
queue.command("cost-budget <id> <usd>").description("Set a queued/failed task USD ceiling; use 0 or unlimited to clear it").action((id: string, usd: string) => {
  const value = /^(?:0|unlimited)$/i.test(usd) ? null : Number(usd);
  if (value !== null && (!Number.isFinite(value) || value <= 0)) throw new Error("Task USD budget must be a positive number, 0, or unlimited.");
  const store = new ResearchStore(statePath);
  const updated = store.setTaskCostBudget(id, value);
  store.close();
  if (!updated) throw new Error(`Task '${id}' is missing or not queued/failed; live work cannot be re-budgeted.`);
...
function parseWorkerCapabilityMap(raw: string | undefined): Map<string, string[]> {
  const capabilities = new Map<string, string[]>();
  for (const entry of (raw ?? "").split(",").map((value) => value.trim()).filter(Boolean)) {
    const separator = entry.indexOf("=");
    const workerId = separator >= 0 ? entry.slice(0, separator).trim() : "";
    const values = separator >= 0 ? entry.slice(separator + 1).split("|").map((value) => value.trim().toLowerCase()).filter(Boolean) : [];
    if (!workerId || workerId.length > 200 || !values.length || values.length > 32 || values.some((value) => value.length > 120 || !/^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(value))) throw new Error("Worker capabilities must use worker-id=capability|capability entries with bounded names.");
    if (capabilities.has(workerId)) throw new Error(`Duplicate worker capability entry for '${workerId}'.`);
    capabilities.set(workerId, [...new Set(values)]);
  }
  return capabilities;
}
...
event.command("serve")
  ...
  .action(async (options: { port: string; host: string; token?: string; tokenFile?: string; taskKinds?: string; workerTokens?: string; workerTokensFile?: string; workerScopes?: string; workerCapabilities?: string }) => {
    ...
    const server = createServer((request, response) => {
      ...
      const operatorAgentDetailPath = Boolean(agentDetailMatch);
      ...
      const agentDirectiveHistoryMatch = request.method === "GET" ? (request.url ?? "").split("?", 1)[0].match(/^\/agents\/([^/]+)\/directives$/) : null;
      const operatorAgentDirectiveHistoryPath = Boolean(agentDirectiveHistoryMatch);
      ...
      if (operatorAttentionPath) {
        ...
        store.close();
        response.writeHead(integrity.status === "invalid" ? 503 : 200, headers);
        response.end(JSON.stringify({ ok: integrity.status !== "invalid", attention, integrity: integrity.status }));
        return;
      }
      if (operatorActivityStreamPath) {
        const query = new URL(request.url ?? "/activity/stream", "http://evidra.local").searchParams;
        const after = Number.parseInt(query.get("after") ?? "0", 10);
        const limit = Number.parseInt(query.get("limit") ?? "50", 10);
        const seconds = Number.parseInt(query.get("seconds") ?? "25", 10);
        if (!Number.isInteger(after) || after < 0 || !Number.isInteger(limit) || limit < 1 || limit > 100 || !Number.isInteger(seconds) || seconds < 1 || seconds > 30) {
          response.writeHead(400, headers);
          response.end(JSON.stringify({ error: "after must be non-negative, limit must be between 1 and 100, and seconds must be between 1 and 30" }));
          return;
        }
        const store = new ResearchStore(statePath);
        const integrity = store.verifyEventChain();
        if (integrity.status === "invalid") {
          ...
          response.end(JSON.stringify({ ok: false, integrity }));
          return;
        }
        const streamHeaders = { ...headers, "cache-control": "no-cache, no-transform", "connection": "keep-alive", "content-type": "text/event-stream; charset=utf-8" };
        response.writeHead(200, streamHeaders);
        response.write(`event: ready\ndata: ${JSON.stringify({ ok: true, after, integrity: integrity.status })}\n\n`);
        let cursor = after;
        let closed = false;
        const finish = () => {
          if (closed) return;
        ...
        const flush = () => {
          if (closed) return;
          const events = store.eventFeed(cursor, limit);
          for (const event of events) {
            cursor = event.id;
            response.write(`event: activity\nid: ${event.id}\ndata: ${JSON.stringify(redactStructured(event))}\n\n`);
          }
        };
        const poll = setInterval(() => {
          if (response.destroyed || response.writableEnded) { finish(); return; }
          flush();
          response.write(`: heartbeat ${new Date().toISOString()}\n\n`);
        }, 250);
        const deadline = setTimeout(finish, seconds * 1_000);
        request.on("close", finish);
        response.on("close", finish);
        flush();
        return;
      }
      if (operatorTaskDetailPath && taskDetailMatch) {
        let taskId: string;
        try { taskId = decodeURIComponent(taskDetailMatch[1] ?? ""); } catch {
          response.writeHead(400, headers);
          response.end(JSON.stringify({ error: "task id must be valid URL encoding" }));
          return;
        }
        const query = new URL(request.url ?? "/tasks/task", "http://evidra.local").searchParams;
        const activityLimit = Number.parseInt(query.get("activityLimit") ?? "32", 10);
        const historyLimit = Number.parseInt(query.get("historyLimit") ?? "64", 10);
        if (!Number.isInteger(activityLimit) || activityLimit < 1 || activityLimit > 128 || !Number.isInteger(historyLimit) || historyLimit < 1 || historyLimit > 128) {
          response.writeHead(400, headers);
          response.end(JSON.stringify({ error: "activityLimit and historyLimit must be between 1 and 128" }));
          return;
        }
        const store = new ResearchStore(statePath);
        ...
        if (!task) {
          ...
          response.end(JSON.stringify({ error: `Unknown queue task '${taskId}'.` }));
          return;
        }
        const { claimToken: _claimToken, ...publicTask } = task;
        const detail = {
          task: publicTask,
          progress: store.queueProgress(taskId),
          readiness: store.taskReadiness(taskId),
          usage: store.queueUsageState(taskId),
          usageTotals: store.queueUsageTotals(taskId),
          checkpoint: store.queueCheckpoint(taskId),
          resume: store.queueResumeContext(taskId, Math.min(16, activityLimit)),
      ...
      if (operatorAgentDetailPath && agentDetailMatch) {
        let role: string;
        try { role = decodeURIComponent(agentDetailMatch[1] ?? ""); } catch {
          response.writeHead(400, headers);
          response.end(JSON.stringify({ error: "agent role must be valid URL encoding" }));
          return;
        }
        const query = new URL(request.url ?? "/agents/role", "http://evidra.local").searchParams;
        const activityLimit = Number.parseInt(query.get("activityLimit") ?? "32", 10);
        const directiveLimit = Number.parseInt(query.get("directiveLimit") ?? "32", 10);
        const sessionLimit = Number.parseInt(query.get("sessionLimit") ?? "16", 10);
        if (![activityLimit, directiveLimit, sessionLimit].every((value) => Number.isInteger(value) && value >= 1 && value <= 128)) {
          response.writeHead(400, headers);
          response.end(JSON.stringify({ error: "activityLimit, directiveLimit, and sessionLimit must be between 1 and 128" }));
          return;
        }
        const store = new ResearchStore(statePath);
        const organizationEntry = agentOrganization(store).find((entry) => entry.role === role);
        ...
        const detail = {
          role: organizationEntry,
          control: store.agentPause(role) ?? null,
          directives: store.agentDirectives(role, directiveLimit),
      ...
      if (operatorAgentDirectiveHistoryPath && agentDirectiveHistoryMatch) {
        let role: string;
        try { role = decodeURIComponent(agentDirectiveHistoryMatch[1] ?? ""); } catch {
          response.writeHead(400, headers);
          response.end(JSON.stringify({ error: "agent role must be valid URL encoding" }));
          return;
        }
        const query = new URL(request.url ?? "/agents/role/directives", "http://evidra.local").searchParams;
        const limit = Number.parseInt(query.get("limit") ?? "32", 10);
        if (!Number.isInteger(limit) || limit < 1 || limit > 128) { response.writeHead(400, headers); response.end(JSON.stringify({ error: "limit must be between 1 and 128" })); return; }
        const store = new ResearchStore(statePath);
        ...
        const directives = redactStructured(store.agentDirectives(role, limit));
        const integrity = store.verifyEventChain();
        store.close();
        response.writeHead(200, headers);
        response.end(JSON.stringify({ ok: integrity.status !== "invalid", role, directives, integrity: integrity.status }));
        return;
      }
      if (operatorOrganizationPath) {
        ...
        const routines = store.routines().slice(0, 64).map((routine) => ({ id: routine.id, name: routine.name, mode: routine.mode, status: routine.status, goal: routine.goal, nextRunAt: routine.nextRunAt, triggerEvent: routine.triggerEvent ?? null, catchUpPolicy: routine.catchUpPolicy ?? "coalesce", pendingTriggers: routine.pendingTriggers ?? 0, droppedTriggers: routine.droppedTriggers ?? 0, lastRunAt: routine.lastRunAt, lastResult: routine.lastResult, lastError: routine.lastError, runCount: routine.runCount, maxRuns: routine.maxRuns }));
      ...
      if (operatorRoutinePath && routineControlMatch) {
        let routineId: string;
        try { routineId = decodeURIComponent(routineControlMatch[1] ?? ""); } catch {
          response.writeHead(400, headers);
          response.end(JSON.stringify({ error: "routine id must be valid URL encoding" }));
          return;
        }
        ...
        try {
          ...
          if (actionName === "recover") {
            ...
            const routine = store.recoverStaleRoutine(routineId);
            if (!before) { response.writeHead(404, headers); response.end(JSON.stringify({ error: `Unknown routine '${routineId}'.` })); return; }
            if (!routine) { response.writeHead(409, headers); response.end(JSON.stringify({ error: `Routine '${routineId}' does not have an expired runner lease.` })); return; }
            response.writeHead(200, headers);
        ...
        try {
          ...
          if (operatorControllerControlPath && controllerControlMatch) {
            ...
            if (!lease) {
              store.close();
              response.writeHead(409, headers);
              response.end(JSON.stringify({ error: "No live controller lease is available." }));
              return;
            }
            store.close();
            response.writeHead(200, headers);
          ...
          if (operatorTaskControlPath && taskControlMatch) {
            let taskId: string;
            try { taskId = decodeURIComponent(taskControlMatch[1] ?? ""); } catch { response.writeHead(400, headers); response.end(JSON.stringify({ error: "task id must be valid URL encoding" })); return; }
            const store = new ResearchStore(statePath);
            const before = store.queueTasks().find((task) => task.id === taskId);
            const action = taskControlMatch[2];
            const changed = action === "pause"
              ? store.pauseTask(taskId, typeof parsed.reason === "string" ? `${remoteActor}: ${parsed.reason}` : `${remoteActor}: remote operator paused task`, remoteActor)
              : store.resumeTask(taskId, remoteActor);
            const after = store.queueTasks().find((task) => task.id === taskId);
            const progress = after ? store.queueProgress(taskId) : undefined;
            store.close();
          ...
          if (operatorTaskAssignPath && taskAssignMatch) {
            let taskId: string;
            try { taskId = decodeURIComponent(taskAssignMatch[1] ?? ""); } catch { response.writeHead(400, headers); response.end(JSON.stringify({ error: "task id must be valid URL encoding" })); return; }
            const rawAssignee = parsed.assigneeId;
            const assigneeId = rawAssignee === null || rawAssignee === undefined || rawAssignee === "" ? null : typeof rawAssignee === "string" ? rawAssignee.trim().slice(0, 200) : "";
            if (rawAssignee !== null && rawAssignee !== undefined && assigneeId === "") throw new Error("assigneeId must be a non-empty string or null.");
            const store = new ResearchStore(statePath);
            if (assigneeId && !agentOrganization(store).some((entry) => entry.role === assigneeId) && !store.externalWorkers(256).some((worker) => worker.workerId === assigneeId)) {
              store.close();
              response.writeHead(409, headers);
              response.end(JSON.stringify({ error: `Unknown or inadmissible assignee '${assigneeId}'.` }));
              return;
            }
            const before = store.queueTasks().find((task) => task.id === taskId);
            const changed = store.assignTask(taskId, assigneeId, remoteActor);
            const after = store.queueTasks().find((task) => task.id === taskId);
            store.close();
            response.writeHead(changed ? 200 : before ? 409 : 404, headers);
            response.end(JSON.stringify(changed ? { ok: true, taskId, assigneeId, status: after?.status ?? null } : { ok: false, taskId, error: before ? "task is not queued or failed" : "task not found" }));
            return;
          }
          ...
          if (taskPath) {
            if (taskPath === "/queue/pause" || taskPath === "/queue/resume") {
              ...
              store.close();
              response.writeHead(200, headers);
              response.end(JSON.stringify({ ok: true, paused: current.paused, reason: current.reason, changed: before.paused !== current.paused || before.reason !== current.reason }));
              return;
            }
            ...
            if (workerCapabilities.size && !workerCapabilities.has(workerId)) {
              response.writeHead(403, headers);
              response.end(JSON.stringify({ error: "worker has no configured capability allowlist" }));
              return;
            }
            const workerAllowedKinds = workerScopes.get(workerId);
            const workerAllowedCapabilities = workerCapabilities.get(workerId);
            ...
            if (taskPath === "/tasks/claim") {
              ...
              const effectiveCapabilities = advertisedCapabilities ?? workerAllowedCapabilities;
              if (workerAllowedCapabilities && effectiveCapabilities?.some((capability) => !workerAllowedCapabilities.includes(capability))) {
                store.close();
                response.writeHead(403, headers);
...
routine.command("list").option("--json", "emit machine-readable routines").action((options: { json?: boolean }) => {
  ...
  else console.log(routines.length ? routines.map((entry) => `${entry.status} ${entry.id} · ${entry.name} · ${entry.mode} · next ${entry.nextRunAt} · every ${entry.intervalSeconds}s · trigger ${entry.triggerEvent ?? "none"} · catch-up ${entry.catchUpPolicy ?? "coalesce"}${entry.pendingTriggers ? ` · pending ${entry.pendingTriggers}` : ""}${entry.droppedTriggers ? ` · dropped ${entry.droppedTriggers}` : ""}${entry.pendingTriggerEvent ? ` · event ${entry.pendingTriggerEvent.eventType} @ ${entry.pendingTriggerEvent.eventCreatedAt}` : ""} · runs ${entry.runCount}${entry.maxRuns !== null ? `/${entry.maxRuns}` : ""}${entry.failureStreak ? ` · failures ${entry.failureStreak}/3${routineRetryDelaySeconds(entry) !== null ? ` · retry backoff ${routineRetryDelaySeconds(entry)}s` : ""}` : ""}${entry.lastResult ? ` · last ${entry.lastResult}` : ""}${entry.lastError ? ` · error ${entry.lastError}` : ""}`).join("\n") : "No routines configured.");
  store.close();
});
...
routine.command("create")
  ...
  .option("--catch-up <policy>", "coalesce missed triggers or replay them (max 8)", "coalesce")
  .action((options: { name: string; goal: string; mode: string; every: string; budget: string; stop: string; provider: string; model: string; thinking: string; autonomy: string; limitPolicy: string; executor: string; lanes: string; maxRuns: string; onEvent?: string; catchUp: string }) => {
    ...
    if (options.catchUp !== "coalesce" && options.catchUp !== "replay") throw new Error("Routine catch-up must be 'coalesce' or 'replay'.");
...
research.command("examples")
  .option("--json", "emit the starter briefs as JSON")
  .description("Show contemporary starter research briefs with metrics and stop rules")
  .action((options: { json?: boolean }) => {
    if (options.json) {
      console.log(JSON.stringify({ briefs: RESEARCH_STARTER_BRIEFS }, null, 2));
      return;
    }
...
research
  ...
  .action(async (options: { mode: string; goal: string; budget: string; gpuBudget: string; agentTokenBudget: string; roleTokenBudgets?: string; stop: string; provider: string; model: string; fallbackModel: string; thinking: string; lanes: string; laneBudget?: string; autonomy: string; limitPolicy: string; executor: string; triggerContext?: string; routineId?: string; resume?: boolean; resumeRun?: string; skipBaseline?: boolean }) => {
    ...
    const routineId = options.routineId ?? savedCampaign?.routineId;
    ...
    if (savedRuntime) {
      ...
      options.roleTokenBudgets = serializeRoleTokenBudgets(savedRuntime.roleTokenBudgets);
    ...
    let triggerContext: { eventType: string; eventCreatedAt: string } | undefined;
    if (options.triggerContext) {
      let parsed: unknown;
      try { parsed = JSON.parse(options.triggerContext); } catch { throw new Error("Trigger context must be valid JSON."); }
      if (!parsed || typeof parsed !== "object" || Array.isArray(parsed) || typeof (parsed as { eventType?: unknown }).eventType !== "string" || typeof (parsed as { eventCreatedAt?: unknown }).eventCreatedAt !== "string" || (parsed as { eventType: string }).eventType.length > 120 || (parsed as { eventCreatedAt: string }).eventCreatedAt.length > 80 || !Number.isFinite(Date.parse((parsed as { eventCreatedAt: string }).eventCreatedAt))) throw new Error("Trigger context must contain a bounded eventType and valid eventCreatedAt.");
      triggerContext = { eventType: (parsed as { eventType: string }).eventType, eventCreatedAt: (parsed as { eventCreatedAt: string }).eventCreatedAt };
    }
    ...
    campaignLoop: do {
      ...
      if (!phaseGoalsForMode(storedGoals, mode, campaignGoalSetId).length) for (const goal of definePhaseGoals(campaign.goal, mode, campaignGoalSetId)) store.savePhaseGoal({ id: goal.id, phase: goal.phase, status: goal.status, payload: goal });
